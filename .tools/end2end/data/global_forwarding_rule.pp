...
#       puppet apply .tools/end2end/data/global_forwarding_rule.pp
#
...
gcompute_global_address { 'puppet-e2e-my-app-lb-address':
  ensure     => present,
...
gcompute_url_map { 'puppet-e2e-my-url-map':
  ensure          => present,
  default_service => 'puppet-e2e-my-app-backend',
  project         => $project, # e.g. 'my-test-project'
  credential      => 'mycred',
}

gcompute_target_http_proxy { 'puppet-e2e-my-http-proxy':
  ensure     => present,
  url_map    => 'puppet-e2e-my-url-map',
  project    => $project, # e.g. 'my-test-project'
  credential => 'mycred',
}

gcompute_global_forwarding_rule { 'puppet-e2e-test1':
  ensure      => present,
