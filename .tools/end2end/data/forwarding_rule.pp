...
#       puppet apply .tools/end2end/data/forwarding_rule.pp
#
...
gcompute_address { 'puppet-e2e-some-address':
  ensure     => present,
...
gcompute_target_pool { 'puppet-e2e-target-pool':
  ensure     => present,
  region     => 'puppet-e2e-some-region',
  project    => $project, # e.g. 'my-test-project'
  credential => 'mycred',
...
gcompute_forwarding_rule { 'puppet-e2e-test1':
  ensure      => present,
  ip_address  => gcompute_address_ref(
    'puppet-e2e-some-address',
    'us-west1', 'google.com:graphite-playground'
  ),
  ip_protocol => 'TCP',
  port_range  => '80',
  target      => 'puppet-e2e-target-pool',
  region      => 'puppet-e2e-some-region',
  project     => $project, # e.g. 'my-test-project'
  credential  => 'mycred',
}
