...
Puppet::Type.type(:gcompute_https_health_check).provide(:google) do
  mk_resource_methods
  ...
  def self.prefetch(resources)
    ...
    resources.each do |name, resource|
      ...
      fetch = fetch_resource(resource, self_link(resource), 'compute#httpsHealthCheck')
      resource.provider = present(name, fetch, resource) unless fetch.nil?
      Google::ObjectStore.instance.add(:gcompute_https_health_check, resource)
    end
  end
  ...
  def resource_to_request
    request = {
      ...
      name: @resource[:name],
      port: @resource[:port],
      requestPath: @resource[:request_path],
