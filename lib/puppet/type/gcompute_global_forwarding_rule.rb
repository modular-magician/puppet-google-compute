...
Puppet::Type.newtype(:gcompute_global_forwarding_rule) do
  @doc = <<-DOC
    Represents a GlobalForwardingRule resource. Global forwarding rules are used to forward traffic
    to the correct load balancer for HTTP load balancing. Global forwarding rules can only be used
    for HTTP load balancing. For more information, see
  ...
  newproperty(:region, parent: Google::Compute::Property::RegionNameRef) do
    desc <<-DOC
      ...
      applicable to global forwarding rules. (output only)
    DOC
  end
  ...
  newproperty(:target, parent: Google::Compute::Property::String) do
    desc <<-DOC
      ...
      appropriate to the target object. Valid types: HTTP_PROXY, HTTPS_PROXY, SSL_PROXY, TCP_PROXY
    DOC
  end
end
